...
/// Similar to TCA's Effect, used for BackgroundStore
///
...
public enum BackgroundEffect<Action: Sendable> {
  case none
  ...
  case merge(id: String? = nil, cancelInFlight: Bool = false, [BackgroundEffect])

  func perform(
    cancellablesDict: inout [String: Set<AnyCancellable>], send: @escaping @Sendable (Action) async -> Void
  ) {
    switch self {
    case .none:
      break
  ...
  public func cancellable(id: String, cancelInFlight: Bool = false) -> BackgroundEffect<Action> {
    switch self {
    case .none, .cancel:
      return self
